...
function kmeans_sequence(p, K, T, St_km, subject_filename, figure_folder)
    t = 1:1:T;
    figureName = sprintf('FS-km-states-p%dK%d-sub%s', p, K, subject_filename);
    figure('Name', figureName, 'Color', [1 1 1]);
    set(gcf, 'PaperPositionMode', 'manual');
    set(gcf, 'PaperUnits', 'inches');
    set(gcf, 'PaperPosition', [0 0 7 2]);
    plot(t, St_km, 'Color', [0 102 204] / 255, 'LineWidth', 2);
    hold on;
    xlim([1 T]);
    ylim([.75 K + .25]);
