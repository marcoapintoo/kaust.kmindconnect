...
def clusters_json(points, labels, centroids, filename, title):
    if np.min(labels) > 0:
        labels = labels - np.min(labels)
    pca = sklearn.decomposition.PCA(n_components=2)
    pca.fit(points)
    cdata = [{'name': 'centroids', 'data': pca.transform(centroids).tolist()}]
    for i, c in enumerate(centroids):
        cdata.append({'name': "cluster {0}".format(i + 1), 'data': pca.transform(points[labels == i]).tolist() })
    json.dump(cdata, open(filename, "wt"))

def clusters(points, labels, centroids, filename, title):
    clusters_json(points, labels, centroids, os.path.splitext(filename)[0] + ".json", title)
    if np.min(labels) > 0:
        labels = labels - np.min(labels)
    pca = sklearn.decomposition.PCA(n_components=2)
    pca.fit(points)
...
def _svg_roi_plot_arrows(arrows, filename_png, brain_file_input, brain_file_output):
...
def _svg_roi_transform_code(fromROI, fromHem, toROI, toHem, strength):
    ROIs = {
        "L": {
            "PreCG": (288, 414),
            "CG": (271, 386),
...
def coherence_matrix_surface(coherence_matrix, file_output, brain_file_reference, convert_to_png=False):
        ...
        for j, fromV in enumerate(order):
            for k2, toH in enumerate(h):
                for i, toV in enumerate(order):
                    arrows.append(_svg_roi_transform_code(fromV, fromH, toV, toH,
                                                            matrix[j + k1 * len(order)][i + k2 * len(order)]))
    _svg_roi_plot_arrows(
