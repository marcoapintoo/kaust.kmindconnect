...
class SVARConnectivityPlotter(IndividualProcessLogger):
    ...
    def after_get_state_space_coefficients(self):
        ...
        print(':: ACTIVITY: Plotting intermediate results')
        path = os.path.join(self.output_folder, "brain_state_sequence") + ".json"
        M = self.associated_process.state_sequence_smoothed.T.ravel().astype("i")
        U = np.unique(M)
        data = []
        for i in U:
            data.append({"name": "State {0}".format(i), "data": [(m, 0) for m in range(len(M))] })
        for n, i in enumerate(M.tolist()):
            #data[i - 1]["data"].append([n, i])
            data[i - 1]["data"][n] = (n, i)
        json.dump(data, open(path, "wt"))
